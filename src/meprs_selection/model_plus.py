...
def compute(models_predict, models_eval):
    forward_scores = []
    ...
    for model_eval in models_eval:
        ...
        for model_predict in models_predict:
            score_file_backward = dataset_file(
                args, model_dict[model_predict] + "_" + args.forward + "_" + model_dict[model_eval] + ".score"
            )
            values = read_scores(score_file_backward)
            backward_scores.append(values)
        backward_scores_list.append(backward_scores)

    backward_scores_zscore = process_sub_lists_avg(backward_scores_list)
...
if __name__ == '__main__':
    ...
    for i in range(1,6):
        for j in range(1,6):
            compute(args.models[:i], args.models[:j])
        print("")
